...
public record CanonicalPlaneRun(
    ...
    /**
     * Instance label map for the inferred slice (RLE), carried through opaquely.
     * The viewer paints it: colour, visibility and opacity are presentation
     * decisions, and shipping a rendered PNG would freeze all three in the backend.
     */
     ...
     *
     * <p>Valid on purpose: the v1 contract never carried one, and a synthetic run has
     * nothing to map. Those planes fall back to the composite overlay, which is what
    ...
    public CanonicalPlaneRun(
        ...
        this(planeRunId, plane, status, effectiveInferenceMode, synthetic, fallbackReason, model, input,
            coordinateSpace, series, assets, masks, landmarks, measurements, null, quality);
    }
