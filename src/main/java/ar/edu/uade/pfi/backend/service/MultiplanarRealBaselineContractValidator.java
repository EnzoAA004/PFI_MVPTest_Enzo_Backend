...
public class MultiplanarRealBaselineContractValidator {
  ...
  public boolean isStrict(MultiplanarRunRequestDto request) {
    if (request == null || request.metadata() == null)
      return Boolean.FALSE.equals(request == null ? null : request.allowContractFallback());
  ...
  public void validate(MultiplanarRunRequestDto request, MultiplanarRunResponseDto response) {
    ...
    requireTrue(
        ...
        "humanReviewRequired root debe ser true");
    requireTrue(
        Boolean.TRUE.equals(response.notClinicalDiagnosis()),
        "notClinicalDiagnosis root debe ser true");
    requireTrue(
        !Boolean.TRUE.equals(response.degradedMode()), "degradedMode root no puede ser true");
   ...
   * (AiMultiplanarV2ResponseDto uses ignoreUnknown=false on critical fields), so the hardcoded v1
   * field-shape assertions below (modelVersion, aiOutput map, metadata keys, etc.) do not apply.
   * Here we only assert the governance guarantees that must hold regardless of contract version.
   */
  private void validateV2Strict(
      MultiplanarRunRequestDto request, MultiplanarRunResponseDto response) {
    ...
    requireTrue(
        !Boolean.TRUE.equals(response.degradedMode()), "degradedMode root no puede ser true");
    require(
        response.planes() != null && response.planes().sagittal() != null,
        "plano sagittal obligatorio");
  }

  public void validateSagittal(
      MultiplanarRunRequestDto request, MultiplanarRunResponseDto.PlaneDto plane) {
    ...
    requireEquals(
        "real_baseline",
        plane.normalizedEffectiveInferenceMode(),
        "effectiveInferenceMode sagittal");
    requireEquals(
        "real_baseline",
        textOr(plane.inferenceMode(), plane.normalizedEffectiveInferenceMode()),
        "inferenceMode sagittal");
    requireEquals(
        "real_baseline", plane.requestedInferenceMode(), "requestedInferenceMode sagittal");
    requireTrue(
        Boolean.FALSE.equals(plane.allowContractFallback()),
        "allowContractFallback sagittal debe ser false");
    if (!blank(request.sagittalInputId()) && !blank(plane.inputId()))
      requireEquals(request.sagittalInputId(), plane.inputId(), "inputId sagittal");
    requireEquals(
        "real_baseline",
        textFrom(plane.aiOutput(), "inferenceMode"),
        "aiOutput.inferenceMode sagittal");
    requireEquals(
        SAGITTAL_ARTIFACT_HASH,
        textFrom(plane.aiOutput(), "artifactHash"),
        "aiOutput.artifactHash sagittal");
    requireTrue(
        Boolean.TRUE.equals(boolFrom(plane.aiOutput(), "realInferenceAvailable")),
        "aiOutput.realInferenceAvailable sagittal debe ser true");
    requireTrue(
        Boolean.TRUE.equals(plane.humanReviewRequired()),
        "humanReviewRequired sagittal debe ser true");
    requireTrue(
        Boolean.TRUE.equals(plane.notClinicalDiagnosis()),
        "notClinicalDiagnosis sagittal debe ser true");
    requireEquals(
        "real_baseline_ready",
        textFrom(plane.measurements(), "status"),
        "measurements.status sagittal");
    requireTrue(
        plane.series() != null && !plane.series().isEmpty(),
        "series sagittal no puede estar vacia");
    requireAsset(plane, "input.png");
    requireAsset(plane, "overlay.png");
    validateSagittalOrientation(plane.metadata());
  }

  public void validateAxial(
      MultiplanarRunRequestDto request, MultiplanarRunResponseDto.PlaneDto plane) {
    ...
    requireEquals(
        "real_baseline", plane.normalizedEffectiveInferenceMode(), "effectiveInferenceMode axial");
    requireTrue(
        Boolean.FALSE.equals(plane.allowContractFallback()),
        "allowContractFallback axial debe ser false");
    if (!blank(plane.inputId()))
      requireEquals(request.axialInputId(), plane.inputId(), "inputId axial");
    requireTrue(
        Boolean.TRUE.equals(boolFrom(plane.aiOutput(), "realInferenceAvailable")),
        "aiOutput.realInferenceAvailable axial debe ser true");
    requireTrue(
        Boolean.TRUE.equals(plane.humanReviewRequired()),
        "humanReviewRequired axial debe ser true");
    requireTrue(
        Boolean.TRUE.equals(plane.notClinicalDiagnosis()),
        "notClinicalDiagnosis axial debe ser true");
  }
  ...
  private void validateSagittalOrientation(Map<String, Object> metadata) {
    ...
    if (listEquals(metadata.get("inputShapeNative"), List.of(17, 512, 512))) {
      requireTrue(
          listEquals(metadata.get("inputShapeCanonical"), List.of(512, 512, 17)),
          "inputShapeCanonical sagittal invalido");
      requireEquals(
          "move_axis_0_to_last",
          text(metadata.get("inputOrientationTransform")),
          "inputOrientationTransform sagittal");
      ...
      requireTrue(
          selectedSlice >= 0 && selectedSlice < 17, "selectedSlice sagittal fuera de rango");
    }
  ...
  private void requireAsset(MultiplanarRunResponseDto.PlaneDto plane, String assetName) {
    require(
        plane.assets() != null && plane.assets().containsKey(assetName),
        "asset " + assetName + " obligatorio en " + plane.plane());
  }
  ...
  private void requireNumberEquals(int expected, Object actual, String field) {
    if (intValue(actual, Integer.MIN_VALUE) != expected)
      fail(field + " esperado=" + expected + " actual=" + actual);
  }
