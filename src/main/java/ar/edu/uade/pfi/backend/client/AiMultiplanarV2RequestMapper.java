...
/**
 * Converts the backend's internal technical request (already stripped of studyMetadata
 * and other identifying fields by the controller) into the typed pfi.multiplanar-run.v2
 * wire contract. Never forwards inputPath, free-form metadata, or backend trace bookkeeping.
 */
@Component
public class AiMultiplanarV2RequestMapper {
    ...
    public AiMultiplanarV2RequestDto toV2Request(MultiplanarRunRequestDto request, String traceId) {
        ...
        require(ALLOWED_INFERENCE_MODES.contains(inferenceMode), "inferenceMode no permitido para el contrato v2: '" + inferenceMode + "'");

        boolean allowContractFallback = Boolean.TRUE.equals(request.allowContractFallback());
        require(!allowContractFallback, "allowContractFallback debe ser false para el contrato v2 en produccion real");

        AiPlaneExecutionV2RequestDto sagittal = plane(request.sagittalInputId(), request.sagittalInputPath(), request.sagittalModelKey(), "sagittal");
        AiPlaneExecutionV2RequestDto axial = plane(request.axialInputId(), request.axialInputPath(), request.axialModelKey(), "axial");
        require(sagittal != null || axial != null, "al menos un plano (sagittal o axial) es obligatorio para el contrato v2");
    ...
    private AiPlaneExecutionV2RequestDto plane(String inputId, String inputPath, String modelKey, String planeName) {
        boolean idPresent = !blank(inputId);
        boolean pathPresent = !blank(inputPath);
        if (!idPresent && !pathPresent) return null;
        if (!idPresent) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
                "El contrato v2 requiere inputId para el plano " + planeName + "; inputPath no es compatible con /v2/multiplanar/run");
        }
        require(!blank(modelKey), "modelKey es obligatorio para el plano " + planeName + " en el contrato v2");
