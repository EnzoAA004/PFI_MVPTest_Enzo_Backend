...
import ar.edu.uade.pfi.backend.config.SecurityStartupValidator;
...
/**
 * Bootstraps exactly one real production ADMIN, once, so that
 * SPRING_PROFILES_ACTIVE=production + PFI_AUTH_DEMO_ENABLED=false never leaves a
 * deployment with zero administrators (the demo account, the only account that used to
 * carry ADMIN by default, is correctly refused in production by P10-A.1 — this fills
 * that gap with a real, deliberately-provisioned account instead).
 *
 * Not a controller: there is no HTTP endpoint for this. It only runs via
 * {@code @PostConstruct}, and only when the active Spring profile is
 * production/prod — every other profile (default/dev/test) is a complete no-op, so
 * `mvn test` never depends on any of the bootstrap-admin-* variables.
 *
 * {@code @DependsOn("securityStartupValidator")} enforces initialization order: (1)
 * SecurityStartupValidator's own @PostConstruct validates structural production config
 * first; only then (2) does this bean even get constructed/its @PostConstruct run,
 * against (3) an already-migrated PostgresAuthStoreService (migration happens in that
 * bean's own constructor, which has already completed by the time it's injected here).
 */
@Component
...
public class AdminBootstrapService {
    ...
    void bootstrap() {
        ...
        DoctorAccount persisted = postgresAuthStore.findByEmail(email)
            .orElseThrow(() -> new IllegalStateException("No se pudo confirmar la persistencia del administrador productivo."));
        if (!persisted.roles().contains("ADMIN")) {
            throw new IllegalStateException("El administrador productivo persistido no contiene el rol ADMIN.");
        }
    ...
    private void validate() {
        ...
        if (password.length() < MIN_PASSWORD_LENGTH) {
            fail("PFI_AUTH_BOOTSTRAP_ADMIN_PASSWORD debe tener al menos " + MIN_PASSWORD_LENGTH + " caracteres");
        }
