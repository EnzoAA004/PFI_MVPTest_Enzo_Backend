...
public class AiModelReadinessResolver {
  ...
  private boolean isModelReady(
      Map<String, Object> response, String modelKey, boolean requireSagittalArtifact) {
  ...
  private String text(Map<String, Object> map, String key) {
    ...
    return value == null
        ? ""
        : value.toString().trim().toLowerCase(Locale.ROOT).equals("null")
            ? ""
            : value.toString().trim();
  }

  public record ModelReadiness(
      boolean sagittalReadyForRealInference, boolean axialReadyForRealInference) {}
}
