...
public class TraceIdFilter extends OncePerRequestFilter {
    ...
    /**
     * Stable, greppable structured line. actorId/roles are added only when AuthFilter
     * already placed them on the request (i.e. the caller was authenticated and
     * revalidated) — never derived from the raw, unrevalidated token here. Never
     * includes email, name, request body, query string, case metadata, file names, or
    ...
    private void logRequest(HttpServletRequest request, String traceId, int status, long elapsedMs) {
        ...
        if (actorId instanceof String actor && !actor.isBlank()) {
            log.info(
                ...
                traceId, request.getMethod(), request.getRequestURI(), status, elapsedMs, outcome, actor, roles
            );
        } else {
            log.info(
                "event=http_request traceId={} method={} path={} status={} durationMs={} outcome={}",
                traceId, request.getMethod(), request.getRequestURI(), status, elapsedMs, outcome
            );
        }
    }
