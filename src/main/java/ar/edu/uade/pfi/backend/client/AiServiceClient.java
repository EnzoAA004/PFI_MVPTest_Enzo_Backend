...
/**
 * The only HTTP client for the AI Module. Never sends the user's JWT/refresh token, email, or roles
 * — this call is backend-to-AI-Module, not a passthrough of the frontend's own credentials. Every
 * call carries an X-Trace-Id header: the current request's trace id (read from MDC, set by
 * TraceIdFilter) when there is one, or a freshly generated {@code technical-<uuid>} — resolved once
 * per call, never stored globally, never leaked across threads — for a background/non-HTTP-request
 * process with nothing in MDC. See docs/P10_B_ERRORS_AUDIT_OBSERVABILITY.md §9/§14.
 *
 * <p>P10-B.1: never builds a public exception message by concatenating the upstream response body,
 * an unwrapped exception's own message, a WebClientResponseException body, or a host/URL — every
 * public message here comes from {@link ApiErrorCode#publicMessage()}. The upstream body is only
 * ever used to deserialize {@link AiStructuredErrorV2Dto} (the v2 structured-error contract); when
 * it isn't structured, it is discarded entirely — not even its length is logged unsanitized.
 */
@Component
public class AiServiceClient implements AiServiceOperations {
  ...
  /**
   * Current request's trace id from MDC, or a fresh {@code technical-<uuid>} — resolved once by the
   * caller and reused for the whole logical call (header + v2 body, where applicable). Never
   * written back to MDC — this is a per-call local value only.
   */
    ...
    return execute(
        () ->
            aiWebClient
                ...
                .uri(
                    uriBuilder ->
                        uriBuilder.path("/models/sync").queryParam("force", force).build())
                .headers(this::applyTraceHeader)
        ...
        () ->
            aiWebClient
                ...
                .uri(
                    uriBuilder ->
                        uriBuilder
                            .path("/assets/{runId}/{plane}/{assetName}")
                            .build(runId, plane, assetName))
                .headers(this::applyTraceHeader)
                .exchangeToMono(
                    response -> {
                            ...
                            .then(
                                Mono.error(
                                    new ResponseStatusException(
                                        response.statusCode(), "AI Module asset request failed")));
                      }
   ...
   *
   * <p>Addressed by input and not by run because these series have no run: they are the ones the
   * study carried and the AI does not analyse — the T1s, the axial T1 with no model, the localizer
   * — and the reader needs them all the same.
   */
    ...
    return execute(
        () ->
            aiWebClient
                ...
                .uri(
                    uriBuilder ->
                        uriBuilder.path("/inputs/{inputId}/slices/{index}").build(inputId, index))
                .headers(this::applyTraceHeader)
                .exchangeToMono(
                    response -> {
                      if (response.statusCode().is2xxSuccessful()) {
                        ...
                        return response
                            ...
                            .then(
                                Mono.error(
                                    new ResponseStatusException(
                                        response.statusCode(),
                                        "AI Module series slice request failed")));
                      }
                      return response.releaseBody().then(Mono.error(upstreamUnavailable()));
                    })
                .block(timeout));
  ...
  public ResponseEntity<byte[]> getRunMeasurementReport(String planeRunId, String plane) {
    return dicomExport(
        "/runs/{runId}/{plane}/measurements.sr.dcm", planeRunId, plane, "measurement report");
  }

  /**
   * Descarga un objeto DICOM construido en el momento por el modulo de IA.
   *
   * <p>El 409 se propaga tal cual y no se convierte en 502: significa que la corrida existe pero no
   * reune las condiciones para exportar —entrada que no es DICOM, mascara vacia, corrida sin
   * mediciones—. Colapsarlo a "el modulo de IA fallo" haria que el visor ofrezca reintentar algo
   * que nunca va a funcionar.
   */
  private ResponseEntity<byte[]> dicomExport(
      String path, String planeRunId, String plane, String what) {
    return execute(
        () ->
            aiWebClient
                .get()
  ...
  /**
   * A-F: resolves the configured contract version, builds the appropriate request, calls exactly
   * one endpoint, deserializes to the matching DTO, adapts to CanonicalMultiplanarRun and returns
   * it. There is no fallback between contracts and no retry against the other endpoint on error.
   */
  @Override
  ...
  /**
   * A call only counts as {@code aiCallsSucceeded} once the whole logical operation — HTTP,
   * deserialization, adapter, contract validation — has completed and produced a usable canonical
   * result (P10-B.1 §12). An HTTP 200 with an invalid contract is a failure, not a success;
   * recorded exactly once, whichever stage throws.
   */
                                  ...
                                  .then(
                                      Mono.error(
                                          new ResponseStatusException(
                                              ...
                                              "El modulo de IA rechazo la solicitud multiplanar.")));
                            }
                            return clientResponse
                                .releaseBody()
                                .then(Mono.error(upstreamUnavailable()));
                          })
                      .block(timeout));
   ...
   *
   * <p>Sigue el mismo camino que {@link #runMultiplanarV2}: trace id propagado por header, error
   * estructurado traducido por tabla fija, y metrica de exito o fallo. La unica diferencia es la
   * tabla de codigos, que es la del contrato subarticular.
   *
   * <p>No valida la respuesta acá. La validacion del contrato clinico vive en {@code
   * DegenerativeFindingsV1Validator} y la aplica el controller, que es quien decide que hacer con
   * un hallazgo mal formado.
   */
  @Override
  public SubarticularPredictionResponseDto predictSubarticular(
      AiSubarticularPredictRequestDto request) {
    String traceId = resolveTraceId();
    long startedAt = System.currentTimeMillis();
    try {
  ...
  /**
   * Igual que {@link #buildV2Error}, pero resolviendo contra la tabla del contrato subarticular. El
   * body crudo nunca se copia a la excepcion publica.
   */
  private RuntimeException buildSubarticularError(String body, String traceId) {
    ...
    AiMultiplanarV2ErrorCodeMapper.Mapped mapped =
        structured != null && structured.code() != null && !structured.code().isBlank()
            ? AiSubarticularErrorCodeMapper.resolve(structured.code())
            : AiMultiplanarV2ErrorCodeMapper.UNKNOWN;
    String aiTraceId =
        structured != null && structured.traceId() != null && !structured.traceId().isBlank()
            ? structured.traceId()
            : traceId;
    return new AiMultiplanarUpstreamException(
        mapped.status(), mapped.backendCode().name(), mapped.publicMessage(), aiTraceId);
  }
  ...
  /**
   * Only ever deserializes the upstream body into {@link AiStructuredErrorV2Dto} — the raw
   * body/message is never copied into the public exception. Technical details (structured code,
   * upstream trace id) are kept only as internal fields on the resulting exception; the public
   * message always comes from {@link ApiErrorCode#publicMessage()} via {@link
   * AiMultiplanarV2ErrorCodeMapper}.
   */
    ...
    return execute(
        () ->
            aiWebClient
                ...
                .uri(
                    uriBuilder ->
                        uriBuilder.path("/agent/reports").queryParam("limit", safeLimit).build())
                .headers(this::applyTraceHeader)
                .retrieve()
                .bodyToMono(MAP_RESPONSE)
                .block(timeout));
   ...
   *
   * <p>Se acota el {@code block} y tambien el {@code responseTimeout} de la conexion, porque cortar
   * solo del lado del que espera deja el socket abierto consumiendo un descriptor hasta que venza
   * el techo largo.
   */
    ...
    return execute(
        () ->
            aiWebClient
                ...
                .httpRequest(
                    request -> {
                      if (request.getNativeRequest()
                          instanceof reactor.netty.http.client.HttpClientRequest nativeRequest) {
  ...
  private Mono<Map<String, Object>> mapResponseOrError(
    ...
    if (response.statusCode().is4xxClientError()) {
      return response
          ...
          .then(
              Mono.error(
                  new ResponseStatusException(
                      response.statusCode(), "La solicitud enviada al modulo de IA es invalida.")));
    }
    return response.releaseBody().then(Mono.error(upstreamUnavailable()));
  }

  private Mono<Map<String, Object>> mapStudyUploadResponseOrError(
      org.springframework.web.reactive.function.client.ClientResponse response,
          ...
          .map(
              entity -> {
                Map<String, Object> body =
                    new LinkedHashMap<>(entity.getBody() == null ? Map.of() : entity.getBody());
                String upstreamTraceId =
                    entity.getHeaders().getFirst(TraceIdFilter.TRACE_ID_HEADER);
                body.put(
                    "traceId",
                    upstreamTraceId == null || upstreamTraceId.isBlank()
                        ? fallbackTraceId
                        : upstreamTraceId);
                return body;
              });
    ...
    if (status == HttpStatus.BAD_REQUEST) {
      return response
          ...
          .then(
              Mono.error(
                  new ResponseStatusException(
                      HttpStatus.BAD_REQUEST, "El estudio enviado al modulo de IA es invalido.")));
    }
          ...
          .then(
              Mono.error(
                  new ResponseStatusException(
                      ...
                      "El estudio supera el tamano maximo permitido.")));
    }
          ...
          .then(
              Mono.error(
                  new ResponseStatusException(
                      ...
                      "El estudio no contiene planos utilizables.")));
    }
    return response.releaseBody().then(Mono.error(upstreamUnavailable()));
  }
  ...
  private MultiplanarRunRequestDto withTraceMetadata(
      MultiplanarRunRequestDto request, String traceId) {
    Map<String, Object> metadata = mergedTraceMetadata(request.metadata(), traceId);
  ...
  /**
   * Simple calls (health/models/assets/...) — the HTTP call itself is the whole logical operation,
   * so success/failure is recorded automatically around it.
   */
  ...
  /**
   * Like {@link #execute}, but without metric recording — used inside {@link #runMultiplanarV1},
   * which records around the full HTTP+validate+adapt pipeline instead.
   */
  ...
  /**
   * Like {@link #executeHttp}, for the v2 dispatch — metric recording happens once, around the full
   * pipeline, in {@link #runMultiplanarV2}.
   */
  private <T> T dispatchV2Http(Supplier<T> supplier, String traceId) {
    try {
      ...
      if (isTimeout(unwrapped)) {
        throw new AiMultiplanarUpstreamException(
            HttpStatus.GATEWAY_TIMEOUT,
            ApiErrorCode.AI_MODULE_TIMEOUT.name(),
            ApiErrorCode.AI_MODULE_TIMEOUT.publicMessage(),
            traceId);
      }
      throw new AiMultiplanarUpstreamException(
          HttpStatus.BAD_GATEWAY,
          ApiErrorCode.AI_MODULE_ERROR.name(),
          ApiErrorCode.AI_MODULE_ERROR.publicMessage(),
          traceId);
    }
  }
  ...
  /**
   * Translates a transport-level failure to a {@link ResponseStatusException} carrying only a fixed
   * public message — never the upstream response body, an unwrapped exception's own message, a
   * host, or a URL. A sanitized, DEBUG-only technical line is logged separately for
   * troubleshooting.
   */
  public ResponseStatusException translateException(RuntimeException ex) {
    ...
    if (isTimeout(unwrapped)) {
      return new ResponseStatusException(
          HttpStatus.GATEWAY_TIMEOUT, ApiErrorCode.AI_MODULE_TIMEOUT.publicMessage(), unwrapped);
    }
    return new ResponseStatusException(
        HttpStatus.BAD_GATEWAY, ApiErrorCode.UPSTREAM_UNAVAILABLE.publicMessage(), unwrapped);
  }
  ...
  private void logTechnicalDetail(Throwable unwrapped) {
    ...
    log.debug(
        "event=ai_module_call_failed exceptionType={} message={}",
