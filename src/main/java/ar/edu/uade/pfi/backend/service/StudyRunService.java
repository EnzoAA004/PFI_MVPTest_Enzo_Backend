...
public class StudyRunService {
  ...
  public Study createStudy(String caseId, String status) {
    ...
    return repository.saveStudy(new Study(UUID.randomUUID().toString(), caseId, status, now, now));
  }
  ...
  public Study upsertStudyMetadata(String caseId, String status, StudyMetadataDto metadata) {
      ...
      Study created =
          saveStudyForMetadata(
              new Study(
                  ...
                  normalized == null
                      ? "medium"
                      : valueOrDefault(normalized.reviewPriority(), "medium"),
                  now,
                  now));
      audit(
          ...
          created,
          normalized,
          Map.of("metadataPresent", normalized != null));
      if (created.subjectRef() != null)
        audit(
            "study.subject_ref.assigned",
            created,
            normalized,
            Map.of("subjectRef", created.subjectRef()));
      return created;
    }
    ...
    LocalDate studyDate =
        normalized.studyDate() == null ? current.studyDate() : normalized.studyDate();
    ...
    if (current.subjectRef() == null && updated.subjectRef() != null) {
      audit(
          ...
          updated,
          normalized,
          Map.of("subjectRef", updated.subjectRef()));
    }
    return updated;
  }

  public InputResource createInput(
      Study study, String plane, String inputId, String format, long size) {
   ...
   *
   * <p>The catalogue holds every series the archive carried, because that is what the doctor reads.
   * {@code analyzable} is what separates the two the models run on from the rest; it is not
   * derivable from the plane, since a sagittal T1 has a plane the registry accepts and still is not
   * what was inferred.
   */
  public InputResource createInput(
      Study study,
  ...
  private void audit(
      String action, Study study, StudyMetadataDto metadata, Map<String, Object> extra) {
    try {
      ...
      if (metadata != null && metadata.subjectRef() != null)
        base.put("subjectRef", metadata.subjectRef());
      base.putAll(extra);
  ...
  public record PreparedStudyMetadata(
      String caseId, StudyMetadataDto metadata, Study existingStudy) {}
}
