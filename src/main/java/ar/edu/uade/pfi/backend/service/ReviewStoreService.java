...
public class ReviewStoreService {
  ...
  private final Map<String, List<MeasurementSaveDto>> measurementsByRunId =
  ...
  public ReviewStatusDto findOrDefault(String runId) {
    return reviews.getOrDefault(
        runId, new ReviewStatusDto(runId, "pendiente", "", "", Instant.now()));
  }
  ...
  public ReviewSnapshotDto snapshot() {
    ...
    return new ReviewSnapshotDto(
        reviews.values().stream()
            .sorted(Comparator.comparing(ReviewStatusDto::updatedAt).reversed())
            .toList(),
        Map.copyOf(measurementsByRunId),
        auditTrail.stream()
            .sorted(Comparator.comparing(AuditEventDto::timestamp).reversed())
            .limit(100)
            .toList());
  }
  ...
  public List<MeasurementSaveDto> saveMeasurements(String runId, MeasurementBatchDto request) {
    List<MeasurementSaveDto> next =
        request.measurements() == null ? List.of() : List.copyOf(request.measurements());
    measurementsByRunId.put(runId, next);
    postgres.saveMeasurements(runId, next);
    appendAudit(
        new AuditEventRequestDto(
  ...
  public ReviewExportResponseDto exportReview(String runId, ReviewExportRequestDto request) {
    ...
    List<MeasurementSaveDto> requestMeasurements =
    ...
    List<MeasurementSaveDto> measurements =
        requestMeasurements.isEmpty() ? storedMeasurements : requestMeasurements;
    ...
    appendAudit(
        new AuditEventRequestDto(
            ...
            "Export de-identificado generado para caseId="
                + context.caseId()
                + ", runId="
                + runId
                + ", format="
  ...
  private String toJson(ExportContext context) {
    try {
              ...
              Map.of(
                  ...
                  "summary",
                      Map.of(
                          ...
                          "outliers",
                              context.measurements().stream()
                                  .filter(item -> Boolean.TRUE.equals(item.outlier()))
                                  .count()),
      ...
      throw new ResponseStatusException(
          HttpStatus.INTERNAL_SERVER_ERROR, "No se pudo generar JSON de export");
    }
  }
          ...
          String.join(
              ...
              csv(
                  measurement.confidence() == null
                      ? ""
                      : Math.round(measurement.confidence() * 100.0) + "%")));
    }
    return "\ufeff" + String.join("\n", rows);
  }
    ...
    String rows =
        context.measurements().stream()
            .map(
                measurement ->
                    "<tr><td><strong>"
                        + html(measurement.label())
                        + "</strong></td><td>"
                        + html(measurement.value())
                        + " "
                        + html(measurement.unit())
                        + "</td><td>"
                        + html(measurement.source())
                        + "</td><td>"
                        + html(measurement.status())
                        + "</td><td>"
                        + (Boolean.TRUE.equals(measurement.outlier()) ? "Si" : "No")
                        + "</td></tr>")
            .reduce("", String::concat);
    return "<!doctype html><html lang=\"es\"><head><meta charset=\"utf-8\"><title>PFI Lumbar MRI - "
        + html(context.caseId())
        + "</title><style>body{font-family:Inter,Segoe UI,Arial,sans-serif;margin:32px;color:#102033;background:#f8fafc}.report{background:#fff;border:1px solid #d8e6f4;border-radius:18px;box-shadow:0 18px 50px rgba(15,23,42,.08);padding:28px;max-width:1080px;margin:auto}.eyebrow{text-transform:uppercase;letter-spacing:.08em;color:#64748b;font-size:12px;font-weight:800}h1{margin:6px 0 4px;font-size:28px}.muted{color:#64748b}.grid{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin:20px 0}.card{border:1px solid #e2e8f0;border-radius:14px;padding:12px;background:#f8fbff}.card strong{display:block;font-size:20px;margin-top:4px}table{border-collapse:collapse;width:100%;margin-top:16px}th{background:#eef4fb;text-align:left;font-size:12px;text-transform:uppercase;color:#475569}td,th{border-bottom:1px solid #e2e8f0;padding:12px;vertical-align:top}.notice{border:1px solid #bae6fd;background:#f0f9ff;border-radius:14px;padding:12px;margin-top:18px}.footer{font-size:12px;color:#64748b;margin-top:20px}@media print{body{background:#fff;margin:0}.report{box-shadow:none;border:0}}</style></head><body><main class=\"report\"><div class=\"eyebrow\">PFI Lumbar MRI Analysis Platform</div><h1>Resumen academico de revision</h1><p class=\"muted\">Caso "
        + html(context.caseId())
        + " · Run "
        + html(context.runId())
        + " · Generado "
        + html(context.generatedAt())
        + "</p><section class=\"grid\"><div class=\"card\">Estado<strong>"
        + html(context.reviewStatus())
        + "</strong></div><div class=\"card\">Modo<strong>"
        + html(context.inferenceMode())
        + "</strong></div><div class=\"card\">Mediciones<strong>"
        + context.measurements().size()
        + "</strong></div><div class=\"card\">Readiness<strong>"
        + html(context.modelReadiness())
        + "</strong></div></section><section class=\"notice\"><strong>Alcance:</strong> uso academico/investigacion, datos de-identificados, requiere revision profesional y no constituye diagnostico clinico. No incluye imagenes crudas.</section><h2>Mediciones revisadas</h2><table><thead><tr><th>Medicion</th><th>Valor</th><th>Fuente</th><th>Estado</th><th>Outlier</th></tr></thead><tbody>"
        + rows
        + "</tbody></table><h2>Notas</h2><p>"
        + html(defaultString(context.notes(), "Sin notas registradas."))
        + "</p><div class=\"footer\">Subject ref de-identificado: "
        + html(context.subjectRef())
        + " · Study date: "
        + html(context.studyDate())
        + " · Modelo: "
        + html(context.modelKey())
        + "</div></main></body></html>";
  }
