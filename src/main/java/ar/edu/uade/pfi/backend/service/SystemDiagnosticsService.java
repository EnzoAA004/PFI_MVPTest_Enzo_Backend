...
public class SystemDiagnosticsService {
  ...
  public Map<String, Object> diagnostics() {
    ...
    result.put(
        "auth",
        authEnabled ? authService.diagnostics() : Map.of("enabled", false, "status", "disabled"));
    result.put(
        "persistence",
        Map.of("mode", persistenceMode, "postgresEnabled", postgresReviewStoreService.enabled()));
    result.put(
        "readiness", aiModule.getOrDefault("readiness", readinessUnavailable("not_checked")));
    result.put("contract", aiModule.getOrDefault("contract", Map.of("status", "unavailable")));
    result.put(
        "modelArtifacts",
        aiModule.getOrDefault("artifactSummary", Map.of("status", "unavailable")));
    result.put(
        "evaluationEvidence",
        aiModule.getOrDefault("evaluationEvidence", evidenceUnavailable("not_checked")));
  ...
  /**
   * In-memory, fixed-shape counters only — see docs/P10_B_ERRORS_AUDIT_OBSERVABILITY.md for the
   * honest limitations (resets on restart, not Cloud Monitoring). Never includes emails, actorIds,
   * caseIds, runIds, individual traceIds, URLs, secrets, paths, file names, or stack traces — every
   * field here is a fixed counter/average.
   */
  ...
  public Map<String, Object> warmup() {
    try {
      ...
      result.put(
          "modelArtifacts",
          response.getOrDefault("artifactSummary", Map.of("status", "unavailable")));
  ...
  /**
   * Never includes the AI Module base URL (may be sensitive infra info) — only the contract
   * version, the endpoint path it resolves to, and the schemaVersion the backend expects back,
   * which is all that's needed to verify a v1/v2 rollout.
   */
  ...
  private Map<String, Object> safeEvidenceSummary() {
    try {
      Map<String, Object> evidence =
          new LinkedHashMap<>(
              DiagnosticsSanitizer.sanitize(aiServiceClient.getEvaluationEvidence()));
  ...
  private Object safeModels() {
    try {
      ...
      return models instanceof Map
          ? DiagnosticsSanitizer.sanitize((Map<String, Object>) models)
          : models;
    } catch (RuntimeException ex) {
