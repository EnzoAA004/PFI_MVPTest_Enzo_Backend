...
/**
 * In-process operational counters — fixed, low-cardinality keys only. This is a stopgap
 * until GCP (Cloud Monitoring) is wired up; see docs/P10_B_ERRORS_AUDIT_OBSERVABILITY.md
 * for the honest limitations (in-memory, resets on redeploy/restart, single instance,
 * not a Prometheus/OTel exporter).
 *
 * Deliberately never keyed by traceId/caseId/runId/email/userId/an arbitrary path, or an
 * exception message — every counter here is a fixed, enumerable field, not a dynamic map
 * keyed by request data.
 */
@Component
public class OperationalMetricsService {
    ...
    public Map<String, Object> snapshot() {
        ...
        snapshot.put("averageRequestDurationMs", average(totalRequestDurationMs.sum(), httpRequestsTotal.sum()));
        snapshot.put("averageAiCallDurationMs", average(aiCallDurationMs.sum(), aiCallCountForAverage.sum()));
