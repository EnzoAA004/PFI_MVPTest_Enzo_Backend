...
public record DegenerativeFindingsV1Dto(
    ...
    List<DegenerativeFindingV1Dto> findings
) {
    @JsonIgnoreProperties(ignoreUnknown = false)
    ...
    public record Classification(
        DegenerativeFindingSeverityV1 label,
        Probabilities probabilities
    ) {}

    @JsonIgnoreProperties(ignoreUnknown = false)
    public record Probabilities(
        @JsonProperty("normal_mild")
        Double normalMild,
        Double moderate,
        Double severe
    ) {}
    ...
    public record Evaluation(
        DegenerativeFindingEvaluationStatusV1 status,
        String reasonCode
    ) {}

    @JsonIgnoreProperties(ignoreUnknown = false)
    public record SourceSeries(
        DegenerativeFindingSourceSeriesRoleV1 role,
        Integer position
    ) {}

    @JsonIgnoreProperties(ignoreUnknown = false)
    public record Localization(
        DegenerativeFindingLocalizationSourceV1 source,
    ...
    public record Review(
        Boolean required,
        DegenerativeFindingReviewStatusV1 status
    ) {}
}
