...
/**
 * Fail-closed startup gate for production profiles. This never runs (and never blocks `mvn test`)
 * unless SPRING_PROFILES_ACTIVE explicitly contains "production"/"prod" — default/dev/test runs are
 * unaffected. In production it refuses to let the context finish starting if any of: the JWT
 * signing key, auth being enabled, demo mode being disabled, dev verification codes being hidden,
 * PostgreSQL persistence being enabled, or CORS being pinned to real HTTPS origins (no wildcard)
 * does not hold.
 */
@Component
public class SecurityStartupValidator {
  ...
  private void validateAuthEnabled() {
    if (!authEnabled) {
      fail(
          "pfi.auth.enabled (PFI_AUTH_ENABLED) must be true in production; "
  ...
  private void validateDevCodesHidden() {
    if (exposeDevCodes) {
      fail(
          "pfi.auth.expose-dev-codes (PFI_AUTH_EXPOSE_DEV_CODES) must be false in production; "
  ...
  private void validateCors() {
    ...
    if (origins.isEmpty()) {
      fail(
          "pfi.cors.allowed-origins (PFI_CORS_ALLOWED_ORIGINS) must configure at least one exact HTTPS origin in production");
    }
    for (String origin : origins) {
      ...
      if (!origin.toLowerCase(Locale.ROOT).startsWith("https://")) {
        fail(
            "pfi.cors.allowed-origins must be exact HTTPS origins in production, found a non-HTTPS entry");
      }
    }
    List<String> patterns = splitTrimmed(corsAllowedOriginPatterns);
    if (!patterns.isEmpty() && !allowPreviewPatterns) {
      fail(
          "pfi.cors.allowed-origin-patterns must be empty in production unless "
