...
public class SagittalRealBaselineContractValidator {
  private static final Set<String> ALLOWED_UPSTREAM_ASSETS =
      Set.of("input.png", "overlay.png", "mask-preview.png", "mask.npy", "confidence.npy");
  ...
  public void validatePipelineResponse(
    ...
    requireEquals(
        "real_baseline", text(response, "requestedInferenceMode"), "requestedInferenceMode");
    ...
    requireEquals(
        "real_baseline",
        text(metadata, "requestedInferenceMode"),
        "metadata.requestedInferenceMode");
  ...
  public void validateSagittalSync(Map<String, Object> response) {
    ...
    requireEquals(
        expected.releaseContentSha256(),
        text(item, "releaseContentSha256"),
        "releaseContentSha256");
    requireEquals(
        expected.releaseManifestSha256(),
        text(item, "releaseManifestSha256"),
  ...
  private Map<String, Object> findSagittalItem(Object value) {
    if (value instanceof Map<?, ?> map) {
      ...
      if (expected.modelKey().equals(text(typed, "modelKey"))
          || expected.modelKey().equals(text(typed, "key"))) {
        return typed;
      }
  ...
  private void validateSeries(Map<String, Object> response, int selectedSlice, int sliceCount) {
    ...
    requireEquals(
        sliceCount,
        intValue(item.get("sliceCount"), "series[0].sliceCount"),
        "series[0].sliceCount");
    requireEquals(
        selectedSlice,
        intValue(item.get("selectedSlice"), "series[0].selectedSlice"),
        "series[0].selectedSlice");
  }
  ...
  private void requireAsset(
      Map<String, Object> assets, String assetName, String runId, String plane) {
  ...
  private void validateInputIdEcho(
      PipelineRunRequestDto request, Map<String, Object> response, Map<String, Object> metadata) {
  ...
  private void validateSpiderNativeShape(
      List<Object> nativeShape,
      List<Object> canonicalShape,
      String transform,
      int selectedAxis,
      int sliceCount) {
