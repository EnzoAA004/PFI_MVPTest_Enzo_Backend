...
class ProfessionalActivationControllerTest {
    ...
    void setUp() {
        ...
        mockMvc = MockMvcBuilders
            ...
            .addFilter(new AuthFilter(tokenService, true, false, new MockEnvironment()))
    ...
    private String bearer(List<String> roles) {
        return "Bearer " + tokenService.issueAccessToken(
            new ar.edu.uade.pfi.backend.auth.DoctorAccount(
                "user-1", "Test User", "user@example.com", "hash", "", "", "", roles,
                java.time.Instant.now(), true, true, false, true
            )
        );
    }
}
