...
class AiInputUploadControllerTest {
  ...
  void uploadInputReturnsInputIdWithoutInternalPaths() throws Exception {
    MockMultipartFile file =
        new MockMultipartFile(
            "file", "case-001-sagittal.npy", "application/octet-stream", new byte[] {1, 2, 3});
  ...
  void uploadInputAcceptsMhaLargerThanOneMbAndBelowTwoHundredMb() throws Exception {
    ...
    MockMultipartFile file =
        new MockMultipartFile("file", "case-001-sagittal.mha", "application/octet-stream", payload);
    when(aiServiceClient.uploadInput(any(), eq("CASE-001"), eq("sagittal")))
        .thenReturn(
            new AiInputResponseDto("input-mha-123", "CASE-001", "sagittal", "mha", payload.length));
  ...
  void uploadInputRejectsUnsupportedExtensionWithSemanticError() throws Exception {
    MockMultipartFile file =
        new MockMultipartFile(
            "file", "case-001.exe", MediaType.APPLICATION_OCTET_STREAM_VALUE, new byte[] {1});
  ...
  void uploadStudyReturnsTypedContractWithoutInternalIdentifiers() throws Exception {
    ...
    when(aiServiceClient.uploadStudy(any(), eq("CASE-001")))
        .thenReturn(
            java.util.Map.of(
                "caseId",
                "CASE-001",
    ...
    mockMvc
        .perform(multipart("/api/ai/studies").file(file).param("caseId", "CASE-001"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.caseId").value("CASE-001"))
  ...
  void maxUploadSizeExceededReturnsInputTooLargeWithoutInternalError() throws Exception {
    MockMvc oversizedMvc =
        MockMvcBuilders.standaloneSetup(new OversizedUploadController())
            .setControllerAdvice(new ApiExceptionHandler())
            .build();

    oversizedMvc
        .perform(post("/api/ai/inputs").header("X-Trace-Id", "trace-upload-too-large"))
