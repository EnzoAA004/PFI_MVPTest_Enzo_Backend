 ...
 *
 * <p>Son lo que permite abrir la segmentacion y las mediciones en 3D Slicer, OHIF o un PACS
 * de hospital. Hasta ahora la exportacion era csv, html o json: formatos que solo entiende
 * este producto.
 */
class AiDicomExportControllerTest {
    ...
    private MockMvc mockMvc(AiServiceOperations ai) {
        AiBackendService service = new AiBackendService(
            ai, Mockito.mock(ReviewStoreService.class), null, null, null, null, null,
            new InMemoryStudyRepository(), null, null, null
        );
        return MockMvcBuilders
            .standaloneSetup(new AiBackendController(service, Mockito.mock(RoleAuthorizationService.class)))
    ...
    void laSegmentacionSeDescargaComoDicom() throws Exception {
        ...
        mockMvc(ai).perform(get("/api/ai/runs/abc123/sagittal/segmentation.dcm"))
    ...
    void lasMedicionesSeDescarganComoDicom() throws Exception {
        ...
        mockMvc(ai).perform(get("/api/ai/runs/abc123/axial/measurements.sr.dcm"))
            .andExpect(status().isOk())
            .andExpect(header().string(HttpHeaders.CONTENT_TYPE, "application/dicom"));
    }
     ...
     *
     * <p>Un Content-Disposition copiado de afuera seria una via para que un nombre elegido
     * por otro sistema llegue al disco del usuario.
     */
    @Test
    void elNombreDelArchivoLoDecideElBackend() throws Exception {
        AiServiceOperations ai = Mockito.mock(AiServiceOperations.class);
        HttpHeaders upstreamHeaders = new HttpHeaders();
        upstreamHeaders.add(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"../../etc/passwd\"");
     ...
     *
     * <p>El objeto se construye en el momento y una corrida revisada puede producir otro.
     * Servir una version vieja de un archivo que alguien va a abrir en otro visor es peor
    ...
    void unPlanoInvalidoSeRechazaSinLlamarAlModuloDeIa() throws Exception {
        ...
        mockMvc(ai).perform(get("/api/ai/runs/abc123/coronal/segmentation.dcm"))
    ...
    void unRunIdConCaracteresDeRutaSeRechaza() throws Exception {
        ...
        mockMvc(ai).perform(get("/api/ai/runs/ab%2F..%2Fhealth/sagittal/segmentation.dcm"))
            .andExpect(status().isBadRequest());
     ...
     *
     * <p>Significa que la corrida existe pero no reune las condiciones para exportar
     * —entrada que no es DICOM, mascara vacia, corrida sin mediciones—. Colapsarlo a "el
     * modulo de IA fallo" haria que el visor ofrezca reintentar algo que nunca va a andar.
     */
    @Test
    void unaCorridaQueNoSePuedeExportarDevuelve409() throws Exception {
        AiServiceOperations ai = Mockito.mock(AiServiceOperations.class);
        Mockito.when(ai.getRunSegmentation(Mockito.anyString(), Mockito.anyString()))
            .thenThrow(new ResponseStatusException(HttpStatus.CONFLICT, "AI Module segmentation export failed"));
