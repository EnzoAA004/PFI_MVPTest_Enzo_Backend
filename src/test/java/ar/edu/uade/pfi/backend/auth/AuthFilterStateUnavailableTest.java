...
/**
 * P10-A.2.1 §3: when Postgres cannot confirm a production session's current state, a protected
 * request must fail closed with 503 AUTH_STATE_UNAVAILABLE — it must never fall through to trusting
 * the JWT's own claims in production.
 */
class AuthFilterStateUnavailableTest {
  ...
  void productionWithUnresolvableAccountStateReturns503() throws Exception {
    ...
    when(store.enabled())
        .thenReturn(
            false); // Postgres disabled in a "production" environment is itself a fail-closed
    // condition.
    ...
    StudyController studyController =
        new StudyController(
            mock(StudyWorklistService.class),
    ...
    MockMvc mockMvc =
        MockMvcBuilders.standaloneSetup(studyController).addFilter(authFilter).build();
