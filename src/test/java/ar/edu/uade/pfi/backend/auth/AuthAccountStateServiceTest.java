...
class AuthAccountStateServiceTest {
  private final TokenService.Claims tokenClaims =
      new TokenService.Claims(
          "user-1", "doc@hospital.example", "Dr X", List.of("DOCTOR", "REVIEWER"));
  ...
  void productionWithQueryFailureIsStateUnavailable() {
    ...
    when(store.findByEmailForAuthorization("doc@hospital.example"))
  ...
  void productionWithMismatchedSubjectIsAccountNotFound() {
    ...
    DoctorAccount persisted =
        account("different-id", "doc@hospital.example", List.of("DOCTOR", "REVIEWER"));
    when(store.findByEmailForAuthorization("doc@hospital.example"))
  ...
  void productionWithDemoAccountIsDemoBlocked() {
    ...
    when(store.enabled()).thenReturn(true);
    DoctorAccount demo =
        account("user-1", AuthService.DEMO_ACCOUNT_EMAIL, List.of("ADMIN", "DOCTOR", "REVIEWER"));
    TokenService.Claims demoTokenClaims =
        new TokenService.Claims("user-1", AuthService.DEMO_ACCOUNT_EMAIL, "Demo", List.of("ADMIN"));
    when(store.findByEmailForAuthorization(AuthService.DEMO_ACCOUNT_EMAIL))
  ...
  void productionEffectiveClaimsComeFromPersistedAccountNotTheToken() {
    ...
    DoctorAccount deactivated =
        account("user-1", "doc@hospital.example", List.of("PENDING_APPROVAL"));
    when(store.findByEmailForAuthorization("doc@hospital.example"))
  ...
  /**
   * P10-A.2.2 §2/§3: approved/verified are authoritative, not just roles — a row left with stale
   * DOCTOR/REVIEWER roles but verified=false must never yield effective claims carrying those
   * roles.
   */
  @Test
  void unverifiedAccountWithStaleProfessionalRolesGetsPendingApprovalEffectiveRoles() {
    PostgresAuthStoreService store = mock(PostgresAuthStoreService.class);
    when(store.enabled()).thenReturn(true);
    DoctorAccount unverified =
        accountWithState(
            "user-1", "doc@hospital.example", List.of("DOCTOR", "REVIEWER"), false, true);
    when(store.findByEmailForAuthorization("doc@hospital.example"))
  ...
  void unapprovedAccountWithStaleProfessionalRolesGetsPendingApprovalEffectiveRoles() {
    PostgresAuthStoreService store = mock(PostgresAuthStoreService.class);
    when(store.enabled()).thenReturn(true);
    DoctorAccount unapproved =
        accountWithState(
            "user-1", "doc@hospital.example", List.of("DOCTOR", "REVIEWER"), true, false);
    when(store.findByEmailForAuthorization("doc@hospital.example"))
  ...
  void unapprovedAdminAccountAlsoGetsPendingApprovalEffectiveRoles() {
    PostgresAuthStoreService store = mock(PostgresAuthStoreService.class);
    when(store.enabled()).thenReturn(true);
    DoctorAccount unapprovedAdmin =
        accountWithState(
            "user-1", "doc@hospital.example", List.of("ADMIN", "DOCTOR", "REVIEWER"), true, false);
    TokenService.Claims adminTokenClaims =
        new TokenService.Claims(
            "user-1", "doc@hospital.example", "Dr X", List.of("ADMIN", "DOCTOR", "REVIEWER"));
    when(store.findByEmailForAuthorization("doc@hospital.example"))
        .thenReturn(Optional.of(unapprovedAdmin));
    AuthAccountStateService service = new AuthAccountStateService(store, production());
  ...
  private DoctorAccount accountWithState(
      String id, String email, List<String> roles, boolean verified, boolean approved) {
