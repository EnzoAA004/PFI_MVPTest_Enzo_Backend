...
/**
 * La ruta que le faltaba a P10.6. Verifica que el backend valide lo que puede validar por si solo,
 * que no deje pasar una respuesta que no cumple el contrato clinico, y que un modulo de IA sin
 * checkpoint se traduzca a un 503 con codigo estable en vez de un 500.
 */
class AiDegenerativeFindingsControllerTest {
  ...
  private MockMvc mockMvc(AiServiceOperations ai) {
    return MockMvcBuilders.standaloneSetup(
            new AiDegenerativeFindingsController(ai, new DegenerativeFindingsV1Validator(), null))
  ...
  void devuelveLasTresProbabilidadesYNormalizaElPedidoAlContratoDelModuloDeIa() throws Exception {
    ...
    mockMvc(ai)
        .perform(
            post("/api/ai/degenerative-findings/subarticular")
        ...
        .andExpect(
            jsonPath("$.degenerativeFindings.findings[0].classification.probabilities.normal_mild")
                .value(0.30))
        .andExpect(
            jsonPath("$.degenerativeFindings.findings[0].classification.probabilities.moderate")
                .value(0.62))
        .andExpect(
            jsonPath("$.degenerativeFindings.findings[0].classification.probabilities.severe")
                .value(0.08))
        .andExpect(
            jsonPath("$.degenerativeFindings.findings[0].localization.researchOnly").value(true))
        .andExpect(jsonPath("$.humanReviewRequired").value(true));

    ArgumentCaptor<AiSubarticularPredictRequestDto> captor =
        ArgumentCaptor.forClass(AiSubarticularPredictRequestDto.class);
  ...
  /**
   * El rechazo no depende del ObjectMapper: lo hace el controller contra el JsonNode. Por eso vale
   * probarlo acá y ademas, con el mapper real, en AiDegenerativeFindingsRealObjectMapperTest — que
   * es el que probaria de verdad si alguien algun dia "simplifica" esto ligando directo al DTO.
   */
  @Test
            ...
            post("/api/ai/degenerative-findings/subarticular")
                ...
                .content(
                    VALID_REQUEST.replace(
                        "\"level\":\"L4-L5\"", "\"level\":\"L4-L5\",\"radius\":8")))
        .andExpect(status().isBadRequest());
  ...
  /**
   * Un 200 con un hallazgo que no cumple el contrato no se muestra a medias: probabilidades que no
   * suman 1 al lado de una barra son indistinguibles de unas que si suman.
   */
  @Test
   ...
   *
   * <p>Sin las probabilidades en el evento, cuando el revisor acepta o corrige un hallazgo lo unico
   * auditable es "el medico acepto algo". Con ellas se puede reconstruir que se le mostro, y medir
   * cuanto se le corrige al modelo.
   */
  @Test
  ...
  void seAuditaLoQueElSistemaMostro() throws Exception {
    ...
    MockMvcBuilders.standaloneSetup(
            new AiDegenerativeFindingsController(ai, new DegenerativeFindingsV1Validator(), audit))
        .setControllerAdvice(new ApiExceptionHandler())
        .build()
        .perform(
            post("/api/ai/degenerative-findings/subarticular")
    ...
    ArgumentCaptor<java.util.Map<String, Object>> metadata =
        ArgumentCaptor.forClass(java.util.Map.class);
    Mockito.verify(audit)
        .record(
            Mockito.eq("backend"),
    ...
    java.util.Map<String, Object> probabilities =
        (java.util.Map<String, Object>) recorded.get("probabilities");
    org.junit.jupiter.api.Assertions.assertEquals(
        0.62d, (Double) probabilities.get("moderate"), 1e-9);
   ...
   *
   * <p>Auditar el resultado es una cosa; convertir el log en una historia clinica paralela es otra.
   * El evento describe una accion del sistema sobre un input, no sobre una persona.
   */
  @Test
  @SuppressWarnings("unchecked")
  void laAuditoriaNoRegistraLaCoordenadaNiIdentificadoresDelPaciente() throws Exception {
    ...
    for (String forbidden :
        java.util.List.of("patientId", "PatientName", "StudyInstanceUID", "SeriesInstanceUID")) {
      org.junit.jupiter.api.Assertions.assertFalse(recorded.containsKey(forbidden), forbidden);
    }
  }

  /**
   * El cliente por defecto de la interfaz lanza UnsupportedOperationException: eso es un 503, no un
   * 500.
   */
  @Test
  void unClienteSinLaOperacionDevuelve503() throws Exception {
    AiServiceOperations ai =
        new AiServiceOperations() {
          @Override
          ...
          @Override
          public java.util.Map<String, Object> verifyModels() {
            return java.util.Map.of();
          }

          @Override
          public java.util.Map<String, Object> warmup() {
            return java.util.Map.of();
          }

          @Override
          public java.util.Map<String, Object> runPipeline(
              ar.edu.uade.pfi.backend.dto.PipelineRunRequestDto request) {
            return java.util.Map.of();
          }

          @Override
          public java.util.Map<String, Object> getAgentReport(String runId) {
            return java.util.Map.of();
          }

          @Override
          public java.util.Map<String, Object> getAgentReportSummary(String runId) {
            return java.util.Map.of();
          }

          @Override
          public java.util.Map<String, Object> getRecentAgentReports(int limit) {
            return java.util.Map.of();
          }
        };
