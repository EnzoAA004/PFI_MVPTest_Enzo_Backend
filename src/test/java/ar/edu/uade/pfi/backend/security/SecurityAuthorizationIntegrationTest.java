 ...
 * representative endpoints, JWT edge cases, CORS, and the standardized 401/403 body shape. Uses the
 * project's existing standaloneSetup + Mockito convention (there is no @SpringBootTest anywhere in
 * this codebase) with the *real* AuthFilter/TokenService/ RoleAuthorizationService wired in front
 * of mocked service dependencies.
 */
class SecurityAuthorizationIntegrationTest {
  ...
  private final AuditService auditService =
      new AuditService(new ar.edu.uade.pfi.backend.repository.InMemoryStudyRepository());
        ...
        MockMvcBuilders.standaloneSetup(
            ...
            .addFilter(
                new CorsResponseFilter(
                    "https://allowed.example.com", "https://*.preview.example.com"))
  ...
  class Anonymous {
    ...
    void assetRequiresAuthentication() throws Exception {
      mockMvc
          .perform(get("/api/ai/assets/run-1/sagittal/overlay.png"))
          .andExpect(status().isUnauthorized());
    }
    ...
    void studyUploadRequiresAuthentication() throws Exception {
      mockMvc
          .perform(multipart("/api/ai/studies").file(zipFile()).param("caseId", "CASE-1"))
          .andExpect(status().isUnauthorized());
    }

    @Test
    void adminDiagnosticsRequiresAuthentication() throws Exception {
      mockMvc.perform(get("/api/system/diagnostics")).andExpect(status().isUnauthorized());
    }

    @Test
    void emptyBearerHeaderIsUnauthorized() throws Exception {
      mockMvc
          .perform(get("/api/studies").header("Authorization", "Bearer "))
          .andExpect(status().isUnauthorized())
          .andExpect(jsonPath("$.code").value("AUTHENTICATION_REQUIRED"));
    }
  }
  ...
  class PendingApproval {
    ...
    void studiesForbidden() throws Exception {
      mockMvc
          .perform(get("/api/studies").header("Authorization", bearer(List.of("PENDING_APPROVAL"))))
  ...
  class ApprovedProfessional {
    ...
    void studiesAllowed() throws Exception {
      when(studyWorklistService.listStudies())
          .thenReturn(
              new StudyListResponseDto("ok", "test", "database", List.of(), Map.of(), true, true));
      mockMvc
          .perform(
              get("/api/studies").header("Authorization", bearer(List.of("DOCTOR", "REVIEWER"))))
    ...
    void studyUploadAllowed() throws Exception {
      when(aiBackendService.uploadStudy(any(), any()))
          .thenReturn(
              new StudyUploadResponseDto(
    ...
    void adminDiagnosticsForbidden() throws Exception {
      mockMvc
          .perform(
              get("/api/system/diagnostics")
                  .header("Authorization", bearer(List.of("DOCTOR", "REVIEWER"))))
          .andExpect(status().isForbidden())
          .andExpect(jsonPath("$.code").value("ACCESS_DENIED"));
    }
  }
  ...
  class Admin {
    ...
    void diagnosticsAllowed() throws Exception {
      ...
      mockMvc
          .perform(get("/api/system/diagnostics").header("Authorization", bearer(List.of("ADMIN"))))
          .andExpect(status().isOk())
    ...
    void adminCanAlsoUseProfessionalEndpoints() throws Exception {
      when(studyWorklistService.listStudies())
          .thenReturn(
              new StudyListResponseDto("ok", "test", "database", List.of(), Map.of(), true, true));
      mockMvc
          .perform(get("/api/studies").header("Authorization", bearer(List.of("ADMIN"))))
          .andExpect(status().isOk());
    }
  }
  ...
  class PublicSurfaceMatrix {
    ...
    void pendingApprovalIsForbiddenFromAiHealthModelsAndWarmup() throws Exception {
      mockMvc
          .perform(
              get("/api/ai/health").header("Authorization", bearer(List.of("PENDING_APPROVAL"))))
          .andExpect(status().isForbidden());
      mockMvc
          .perform(
              get("/api/ai/models").header("Authorization", bearer(List.of("PENDING_APPROVAL"))))
          .andExpect(status().isForbidden());
      mockMvc
          .perform(
    ...
    void approvedProfessionalCanReadAiHealthAndModelsButNotWarmup() throws Exception {
      ...
      mockMvc
          .perform(get("/api/ai/health").header("Authorization", bearer(List.of("DOCTOR"))))
          .andExpect(status().isOk());
      mockMvc
          .perform(get("/api/ai/models").header("Authorization", bearer(List.of("DOCTOR"))))
          .andExpect(status().isOk());
      mockMvc
          .perform(post("/api/system/warmup").header("Authorization", bearer(List.of("DOCTOR"))))
          .andExpect(status().isForbidden());
    }
    ...
    void adminCanTriggerWarmup() throws Exception {
      ...
      mockMvc
          .perform(post("/api/system/warmup").header("Authorization", bearer(List.of("ADMIN"))))
          .andExpect(status().isOk());
    }
  }
  ...
  class ReadinessAndVerifyAreAdminOnly {
    ...
    void approvedProfessionalIsForbidden() throws Exception {
      mockMvc
          .perform(get("/api/ai/readiness").header("Authorization", bearer(List.of("DOCTOR"))))
          .andExpect(status().isForbidden());
      mockMvc
          .perform(get("/api/ai/models/verify").header("Authorization", bearer(List.of("DOCTOR"))))
          .andExpect(status().isForbidden());
    }
    ...
    void adminIsAllowed() throws Exception {
      ...
      mockMvc
          .perform(get("/api/ai/readiness").header("Authorization", bearer(List.of("ADMIN"))))
          .andExpect(status().isOk());
      mockMvc
          .perform(get("/api/ai/models/verify").header("Authorization", bearer(List.of("ADMIN"))))
          .andExpect(status().isOk());
    }
  }
  ...
  class JwtEdgeCases {
    ...
    void expiredTokenIsUnauthorized() throws Exception {
      ...
      mockMvc
          .perform(get("/api/studies").header("Authorization", "Bearer " + token))
          .andExpect(status().isUnauthorized());
    }
    ...
    void invalidSignatureIsUnauthorized() throws Exception {
      ...
      mockMvc
          .perform(get("/api/studies").header("Authorization", "Bearer " + tampered))
          .andExpect(status().isUnauthorized());
    }
    ...
    void malformedTokenIsUnauthorized() throws Exception {
      mockMvc
          .perform(get("/api/studies").header("Authorization", "Bearer not-a-jwt"))
          .andExpect(status().isUnauthorized());
    }
    ...
    void tokenWithoutAuthoritiesIsDeniedNotGrantedDefault() throws Exception {
      ...
      mockMvc
          .perform(get("/api/system/diagnostics").header("Authorization", "Bearer " + token))
          .andExpect(status().isForbidden());
    }
  }
  ...
  class Cors {
    ...
    void wildcardPatternMatchesPreviewSubdomain() throws Exception {
      ...
      mockMvc
          ...
          .andExpect(
              header().string("Access-Control-Allow-Origin", "https://pr-42.preview.example.com"));
    }
  }
  ...
  class ResponseSafety {
    ...
    void unauthorizedBodyNeverLeaksTokenOrStackTrace() throws Exception {
      String body =
          mockMvc
              .perform(get("/api/studies").header("Authorization", "Bearer garbage"))
              .andReturn()
              .getResponse()
              .getContentAsString();
    ...
    void forbiddenBodyMatchesStandardShape() throws Exception {
      mockMvc
          .perform(
              get("/api/system/diagnostics").header("Authorization", bearer(List.of("DOCTOR"))))
  ...
  /**
   * Builds a raw JWT with the same HS256 scheme as TokenService, for edge-case payloads
   * TokenService itself never issues.
   */
  ...
  private String base64Json(Map<String, Object> value) throws Exception {
    return Base64.getUrlEncoder()
        .withoutPadding()
        .encodeToString(objectMapper.writeValueAsBytes(new LinkedHashMap<>(value)));
  }
