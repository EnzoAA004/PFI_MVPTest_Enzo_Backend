...
/**
 * P10-B.2: GET /api/ai/models/runtime previously had no authorization check beyond AuthFilter's
 * blanket "any authenticated professional" gate — this is the HTTP-layer matrix proving
 * anonymous->401, DOCTOR/REVIEWER->403, ADMIN->200 now hold end to end through the real AuthFilter
 * + RoleAuthorizationService, not just at the unit level covered by AiModelRuntimeControllerTest.
 */
class AiModelRuntimeControllerAuthorizationTest {
  ...
  void setUp() {
    ...
    when(client.getModelRuntime())
        .thenReturn(Map.of("status", "pytorch_runtime_ready", "device", "cpu"));
  ...
  void anonymousIsUnauthorized() throws Exception {
    mockMvc
        .perform(get("/api/ai/models/runtime"))
  ...
  void doctorIsForbidden() throws Exception {
    mockMvc
        .perform(get("/api/ai/models/runtime").header("Authorization", bearer(List.of("DOCTOR"))))
        .andExpect(status().isForbidden())
  ...
  void reviewerIsForbidden() throws Exception {
    mockMvc
        .perform(get("/api/ai/models/runtime").header("Authorization", bearer(List.of("REVIEWER"))))
        .andExpect(status().isForbidden());
  }
  ...
  void adminIsAllowed() throws Exception {
    mockMvc
        .perform(get("/api/ai/models/runtime").header("Authorization", bearer(List.of("ADMIN"))))
  ...
  void adminGetsUniform503WhenAiModuleRuntimeFails() throws Exception {
    when(client.getModelRuntime())
        .thenThrow(
            new RuntimeException("http://ai-module:8000/models/runtime refused C:\\secret\\stack"));
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath("$.message")
                .value(
                    org.hamcrest.Matchers.not(org.hamcrest.Matchers.containsString("ai-module"))))
        .andExpect(
            jsonPath("$.message")
