...
class StudyMetadataControllerTest {
  ...
  void putMetadataRequiresProfessionalAndUpdatesExistingStudy() throws Exception {
    ...
    mockMvc
        .perform(
            put("/api/studies/CASE-META/metadata")
                .with(reviewer())
  ...
  void putMetadataKeepsStatusUsesNullForMissingFieldsAndRejectsUnknownPriority() throws Exception {
    ...
    String response =
        mockMvc
            .perform(
                put("/api/studies/CASE-READY-META/metadata")
                    .with(reviewer())
                    .contentType(MediaType.APPLICATION_JSON)
    ...
    org.junit.jupiter.api.Assertions.assertEquals(
        "ready", repository.findStudyByCaseId("CASE-READY-META").orElseThrow().status());
    ...
    return request -> {
      request.setAttribute(
          ...
          new TokenService.Claims(
              "reviewer-id", "reviewer@example.test", "Reviewer", List.of("REVIEWER")));
      return request;
    };
  }
}
