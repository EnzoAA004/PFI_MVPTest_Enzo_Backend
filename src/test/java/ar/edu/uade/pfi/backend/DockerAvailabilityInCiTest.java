 ...
 * {@code @Testcontainers(disabledWithoutDocker = true)}: en una maquina sin Docker se saltean en
 * vez de romper el build. Eso es lo que queremos localmente y es exactamente lo que no queremos en
 * CI.
 *
 * <p>Sin este chequeo, un runner que pierda Docker dejaria de correr las ~52 pruebas de
 * persistencia y el build seguiria en verde: la senal mas peligrosa de todas, porque no se
 * distingue de un build que si las corrio.
 *
 * <p>Este test solo existe donde la variable {@code CI} esta definida, que es lo que hace GitHub
 * Actions. En local no aparece.
 */
