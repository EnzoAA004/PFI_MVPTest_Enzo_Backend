...
class SecurityStartupValidatorTest {
  private static final String STRONG_SECRET =
      "a-sufficiently-long-random-production-secret-value-1234";
  ...
  void nonProductionProfileNeverBlocksStartupRegardlessOfConfig() {
    ...
    assertDoesNotThrow(
        () -> validator(env, "", false, true, true, "memory", "", "*", true).validate());
  }
  ...
  void productionWithBlankSecretFailsStartup() {
    assertThrows(
        ...
        () ->
            validator(production(), "", true, false, false, "postgres", EXACT_ORIGIN, "", false)
                .validate());
  }
  ...
  void productionWithoutHttpsOriginFailsStartup() {
    assertThrows(
        IllegalStateException.class,
        () ->
            validator(production(), STRONG_SECRET, true, false, false, "postgres", "", "", false)
                .validate());
  }
