...
/**
 * El endpoint del modulo de IA declara {@code extra="forbid"}: un campo de mas y rechaza el pedido
 * entero. Si el backend lo dejara pasar, el cliente se enteraria dos saltos despues y con un codigo
 * que no dice nada.
 *
 * <p>Va contra el ObjectMapper real que administra Spring Boot y no contra uno armado a mano en un
 * {@code standaloneSetup}, por el mismo motivo que {@code
 * ProfessionalActivationRealObjectMapperTest}: el mapper de standalone no es el que usa la
 * aplicacion, asi que probar el rechazo ahi no prueba nada sobre produccion.
 *
 * <p>{@code AuthFilter} queda afuera de la rebanada: necesita Postgres y tokens, y no tiene nada
 * que ver con si el DTO rechaza un campo desconocido.
 */
@WebMvcTest(
...
class AiDegenerativeFindingsRealObjectMapperTest {
  ...
  @MockBean private AiServiceOperations aiServiceClient;
