...
/**
 * Un campo desconocido en un payload del cliente se rechaza, y se rechaza con el ObjectMapper que
 * usa la aplicacion.
 *
 * <p><b>Por que existe este test.</b> ~25 DTOs de este repo llevan
 * {@code @JsonIgnoreProperties(ignoreUnknown = false)} como control deliberado, y varios javadocs
 * afirman que asi frenan payloads que intentan colar {@code roles}, {@code admin} o {@code
 * password}. Se verifico empiricamente que <b>eso no era cierto</b>: Spring Boot deja {@code
 * FAIL_ON_UNKNOWN_PROPERTIES} apagado, y la anotacion sola no fuerza el fallo —solo declina optar
 * por ignorar—. La proteccion existia en el papel.
 *
 * <p>Con un {@code new ObjectMapper()} armado a mano el mismo DTO si rechaza, que es exactamente
 * como el problema pasaba desapercibido: un test de rechazo escrito sobre un mapper propio pasa y
 * no prueba nada sobre produccion. Por eso este test usa {@code @JsonTest}, que trae el mapper
 * autoconfigurado real.
 *
 * <p>La lista de campos peligrosos incluye los que pide el handoff de P10.7 §9.4: ademas de los de
 * escalada de privilegios, {@code checkpointPath}, {@code patientId} y los UIDs DICOM, que nunca
 * deben viajar en un request publico.
 */
@JsonTest
class UnknownFieldRejectionAuditTest {
  ...
  void laActualizacionDeAjustesNoAceptaCamposDeMas() {
    assertThrows(
        ...
        () ->
            mapper.readValue(
                "{\"twoFactorEnabled\":true,\"roles\":[\"ADMIN\"]}",
                AuthDtos.SettingsRequest.class));
  }
  ...
  /**
   * P10.7 §7: el request publico no debe llevar rutas de checkpoint, identificadores de paciente ni
   * UIDs DICOM. Se verifica sobre el DTO del pedido subarticular, que es el request publico mas
   * nuevo.
   */
  @ParameterizedTest
  @ValueSource(
  ...
  /**
   * El payload exacto sigue entrando. Un rechazo que ademas rompe el caso bueno no es una
   * proteccion, es una caida.
   */
  @Test
  void losPayloadsExactosSiguenSiendoValidos() {
    assertDoesNotThrow(
        () -> {
          mapper.readValue(
              "{\"email\":\"doc@example.com\",\"password\":\"Demo1234!\"}",
              AuthDtos.LoginRequest.class);
          mapper.readValue(
              "{\"twoFactorEnabled\":true,\"onboardingCompleted\":false}",
              AuthDtos.SettingsRequest.class);
          mapper.readValue(
   ...
   *
   * <p>El sobre de una respuesta del modulo de IA transporta metadata operativa que va a seguir
   * creciendo. Romper una corrida entera porque aparecio una clave nueva ahi es lo que ya paso con
   * {@code discLevels}. La bandera global no puede volver a provocarlo: {@code ignoreUnknown =
   * true} manda por encima de ella.
   */
  @Test
