...
/**
 * P10-B §1/§18-A: every error response — regardless of the triggering exception —
 * carries the full, uniform contract, including category/retryable/governance flags.
 */
class ApiExceptionHandlerContractTest {
    ...
    void unauthorizedHasFullContract() throws Exception {
        assertFullContract("/boom/401", status().isUnauthorized(), "AUTHENTICATION_REQUIRED", "AUTHENTICATION");
    }
    ...
    void notFoundHasFullContract() throws Exception {
        assertFullContract("/boom/study-not-found", status().isNotFound(), "STUDY_NOT_FOUND", "RESOURCE");
    }

    @Test
    void databaseUnavailableIsRetryable() throws Exception {
        mockMvc.perform(get("/boom/db-unavailable"))
    ...
    void aiContractViolationIsNeverRetryable() throws Exception {
        mockMvc.perform(get("/boom/ai-contract"))
    ...
    void aiMultiplanarContractViolationCategory() throws Exception {
        mockMvc.perform(get("/boom/ai-multiplanar-contract"))
            .andExpect(status().isBadGateway())
    ...
    static class ErrorController {
        ...
        void runtime() {
            throw new RuntimeException("jdbc:postgresql://synthetic_user:synthetic_pw@internal-db:5432/pfi connection refused");
        }
    }
}
