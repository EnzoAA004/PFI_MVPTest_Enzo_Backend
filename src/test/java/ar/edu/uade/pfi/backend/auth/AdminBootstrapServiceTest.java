...
class AdminBootstrapServiceTest {
    ...
    void createdAdminHasExpectedRolesAndFlags() {
        ...
        org.mockito.ArgumentCaptor<DoctorAccount> captor = org.mockito.ArgumentCaptor.forClass(DoctorAccount.class);
        verify(store).createBootstrapAdmin(captor.capture());
        DoctorAccount created = captor.getValue();
        org.junit.jupiter.api.Assertions.assertEquals(List.of("ADMIN", "DOCTOR", "REVIEWER"), created.roles());
    ...
    void emptyFullNameIsRejected() {
        ...
        when(store.hasNonDemoAdmin(AuthService.DEMO_ACCOUNT_EMAIL)).thenReturn(false);
        assertThrows(IllegalStateException.class,
            () -> new AdminBootstrapService(store, passwordHasher, production(), true, VALID_EMAIL, VALID_PASSWORD, "", VALID_LICENSE, VALID_INSTITUTION, JWT_SECRET).bootstrap());
    }
    ...
    void emptyLicenseIsRejected() {
        ...
        when(store.hasNonDemoAdmin(AuthService.DEMO_ACCOUNT_EMAIL)).thenReturn(false);
        assertThrows(IllegalStateException.class,
            () -> new AdminBootstrapService(store, passwordHasher, production(), true, VALID_EMAIL, VALID_PASSWORD, VALID_NAME, "", VALID_INSTITUTION, JWT_SECRET).bootstrap());
    }
    ...
    void emptyInstitutionIsRejected() {
        ...
        when(store.hasNonDemoAdmin(AuthService.DEMO_ACCOUNT_EMAIL)).thenReturn(false);
        assertThrows(IllegalStateException.class,
            () -> new AdminBootstrapService(store, passwordHasher, production(), true, VALID_EMAIL, VALID_PASSWORD, VALID_NAME, VALID_LICENSE, "", JWT_SECRET).bootstrap());
    }

    @Test
    void postgresDisabledSurfacesAsStartupFailure() {
        PostgresAuthStoreService store = mock(PostgresAuthStoreService.class);
        when(store.hasNonDemoAdmin(anyString())).thenThrow(new IllegalStateException("Postgres auth persistence is not enabled"));

        assertThrows(IllegalStateException.class, () -> service(store, true, VALID_EMAIL, VALID_PASSWORD).bootstrap());
    }

    @Test
    void connectionFailureSurfacesAsStartupFailure() {
        PostgresAuthStoreService store = mock(PostgresAuthStoreService.class);
        when(store.hasNonDemoAdmin(anyString())).thenThrow(new IllegalStateException("Could not query for an existing production ADMIN account"));
    ...
    void persistenceErrorSurfacesAsStartupFailure() {
        PostgresAuthStoreService store = mock(PostgresAuthStoreService.class);
        when(store.hasNonDemoAdmin(AuthService.DEMO_ACCOUNT_EMAIL)).thenReturn(false);
        when(store.createBootstrapAdmin(any())).thenThrow(new IllegalStateException("Could not persist the bootstrap ADMIN account"));
    ...
    private DoctorAccount persistedAdmin() {
        return new DoctorAccount(
            "bootstrap-1", VALID_NAME, VALID_EMAIL, passwordHasher.hash(VALID_PASSWORD),
            VALID_LICENSE, "", VALID_INSTITUTION, List.of("ADMIN", "DOCTOR", "REVIEWER"),
            Instant.now(), true, true, false, false
        );
    }
    ...
    private AdminBootstrapService service(PostgresAuthStoreService store, boolean bootstrapEnabled, String email, String password) {
        return new AdminBootstrapService(
            store, passwordHasher, production(), bootstrapEnabled, email, password,
            VALID_NAME, VALID_LICENSE, VALID_INSTITUTION, JWT_SECRET
        );
    }
}
