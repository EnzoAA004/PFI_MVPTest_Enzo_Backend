...
class RunAssetSnapshotServiceTest {
  ...
  void snapshotsInputAndOverlayPngButNeverTreatsMaskOrConfidenceAsPng() {
    ...
    List<RunArtifact> artifacts =
        List.of(
            ...
            artifact(
                study.id(), "run-sag-snap", "sagittal", "mask.npy", "application/octet-stream"),
            artifact(
                study.id(),
                "run-sag-snap",
                "sagittal",
    ...
    byte[] pngBytes = png(1);
    when(ai.getAsset(eq("run-sag-snap"), eq("sagittal"), eq("input.png")))
        .thenReturn(pngResponse(pngBytes));
    when(ai.getAsset(eq("run-sag-snap"), eq("sagittal"), eq("overlay.png")))
        .thenReturn(pngResponse(pngBytes));
  ...
  void snapshotsWorkspaceLumbarMeshJsonAsDurablePublicAsset() {
    ...
    AiServiceOperations ai = mock(AiServiceOperations.class);
    byte[] body = meshJson();
    when(ai.getAsset(eq("multi-3d-snapshot"), eq("workspace"), eq("lumbar-3d-mesh.json")))
        .thenReturn(jsonResponse(body));
    FakeRunAssetContentStorage storage = new FakeRunAssetContentStorage();
  ...
  void rejectsPngAssetWhenUpstreamReturnsJsonOrNonPngBytes() {
    ...
    when(ai.getAsset(eq("run-bad-png"), eq("sagittal"), eq("input.png")))
        .thenReturn(
            jsonResponse(
                "{\"html\":\"not-a-png\"}".getBytes(java.nio.charset.StandardCharsets.UTF_8)));

    new RunAssetSnapshotService(ai, repository, new FakeRunAssetContentStorage(), null, 5_242_880)
        .snapshot(run);

    RunArtifact reloaded =
        repository.findRunByMultiplanarRunId("multi-bad-png").orElseThrow().artifacts().get(0);
  ...
  void rejectsUnsafeMeshJsonContract() {
    ...
    RunArtifact mesh =
        artifact(
            study.id(), "multi-bad-mesh", "workspace", "lumbar-3d-mesh.json", "application/json");
    StudyRun run =
        studyRunService.createRunWithId(
            UUID.randomUUID().toString(),
            study,
    ...
    when(ai.getAsset(eq("multi-bad-mesh"), eq("workspace"), eq("lumbar-3d-mesh.json")))
        .thenReturn(jsonResponse(unsafe));

    new RunAssetSnapshotService(ai, repository, new FakeRunAssetContentStorage(), null, 5_242_880)
        .snapshot(run);

    RunArtifact reloaded =
        repository.findRunByMultiplanarRunId("multi-bad-mesh").orElseThrow().artifacts().get(0);
    assertEquals("rejected", reloaded.storageStatus());
  }

  private RunArtifact artifact(
      String studyId, String runId, String plane, String assetName, String contentType) {
  ...
  private static final class FakeRunAssetContentStorage implements RunAssetContentStorage {
    ...
    public RunAssetContent store(RunArtifact artifact, byte[] content, String sha256) {
      RunAssetContent value =
          new RunAssetContent(
              artifact.id(), content, sha256, content.length, "fake", Instant.now());
