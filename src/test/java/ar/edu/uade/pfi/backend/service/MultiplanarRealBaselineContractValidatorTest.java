...
class MultiplanarRealBaselineContractValidatorTest {
  ...
  void strictRealBaselineSagittalOnlyValidResponsePassesWithAxialUnavailable() throws Exception {
    ...
    assertDoesNotThrow(
        () -> validator.validate(strictSagittalOnlyRequest(), presenter.present(read(response))));
  }
  ...
  void axialIsValidatedIndependentlyWithoutSagittalShaRules() throws Exception {
    assertDoesNotThrow(
        () -> validator.validateAxial(strictRequest(), presentedFixture().planes().axial()));
    ...
    assertDoesNotThrow(
        () -> {
          ...
          validator.validateAxial(
              strictRequest(), presenter.present(read(response)).planes().axial());
        });
  }
  ...
  void rootDegradedOrUnsafeClinicalFlagsFailButMixedWorkspaceIsAllowed() throws Exception {
    assertDoesNotThrow(
        () ->
            validator.validate(
                strictRequest(),
                presenter.present(read(mutateRoot("effectiveInferenceMode", "mixed")))));
  ...
  void v2SchemaVersionSkipsV1SpecificFieldShapeChecksButStillEnforcesGovernance() {
    MultiplanarRunResponseDto v2Shaped =
    ...
    MultiplanarRunResponseDto missingGovernance =
        new MultiplanarRunResponseDto(
            v2Shaped.status(),
            v2Shaped.schemaVersion(),
            v2Shaped.runId(),
            v2Shaped.traceId(),
            v2Shaped.caseId(),
            v2Shaped.workspaceMode(),
            v2Shaped.requestedInferenceMode(),
            v2Shaped.effectiveInferenceMode(),
            v2Shaped.planes(),
            v2Shaped.assets(),
            v2Shaped.threeD(),
            v2Shaped.quality(),
    ...
    assertThrows(
        ...
        () -> validator.validate(strictSagittalOnlyRequest(), missingGovernance));
  }
  ...
  private void assertFails(Map<String, Object> response) {
    assertThrows(
        AiMultiplanarContractViolationException.class,
        () -> validator.validate(strictRequest(), presenter.present(read(response))));
  }
