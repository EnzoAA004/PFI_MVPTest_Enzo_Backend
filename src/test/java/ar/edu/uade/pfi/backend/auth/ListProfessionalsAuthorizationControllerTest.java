 ...
 * RoleAuthorizationService.requireAdmin gate in addition to AuthService's own internal
 * check — this proves the HTTP-layer matrix (anonymous->401, DOCTOR/REVIEWER->403,
 * ADMIN->200) holds even if the service-level check were ever removed or bypassed.
 */
class ListProfessionalsAuthorizationControllerTest {
    ...
    void setUp() {
        ...
        mockMvc = MockMvcBuilders
            ...
            .addFilter(new AuthFilter(
                tokenService,
                new AuthAccountStateService(mock(PostgresAuthStoreService.class), new MockEnvironment()),
    ...
    void anonymousIsUnauthorized() throws Exception {
        mockMvc.perform(get("/api/auth/admin/professionals"))
    ...
    void pendingApprovalIsForbidden() throws Exception {
        mockMvc.perform(get("/api/auth/admin/professionals").header("Authorization", bearer(List.of("PENDING_APPROVAL"))))
            .andExpect(status().isForbidden());
    }

    @Test
    void doctorIsForbidden() throws Exception {
        mockMvc.perform(get("/api/auth/admin/professionals").header("Authorization", bearer(List.of("DOCTOR"))))
            .andExpect(status().isForbidden())
    ...
    void reviewerIsForbidden() throws Exception {
        mockMvc.perform(get("/api/auth/admin/professionals").header("Authorization", bearer(List.of("REVIEWER"))))
            .andExpect(status().isForbidden());
    }

    @Test
    void adminIsAllowed() throws Exception {
        when(authService.listProfessionals(any())).thenReturn(List.of(
            new UserResponse("id-1", "Dr Doc", "doc@hospital.example", "MN-1", "Spine", "Hospital",
                List.of("DOCTOR", "REVIEWER"), true, true, false, true)
        ));

        mockMvc.perform(get("/api/auth/admin/professionals").header("Authorization", bearer(List.of("ADMIN"))))
