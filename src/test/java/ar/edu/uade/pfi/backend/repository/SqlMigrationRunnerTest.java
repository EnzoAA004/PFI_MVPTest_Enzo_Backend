 ...
 *
 * <p>La ruta de las migraciones es relativa al working directory. Cuando no resolvia, el
 * runner devolvia una lista vacia: el servicio levantaba contra un esquema inexistente y
 * fallaba recien en la primera consulta, con un error que no menciona migraciones. Un
 * arranque que no puede construir el esquema tiene que morir donde esta el problema.
 *
 * <p>No necesitan base: la validacion del directorio ocurre antes de tocar la conexion,
 * que es justamente lo que se quiere —fallar antes de abrir nada—.
 */
class SqlMigrationRunnerTest {
    ...
    void unDirectorioInexistenteFallaYDiceCualBuscoYDesdeDonde(@TempDir Path tmp) {
        ...
        SqlMigrationRunner.MigrationsUnavailableException ex = assertThrows(
    ...
    /**
     * El default es el que usan los dos componentes que migran. Si alguien lo cambia sin
     * tocar el Dockerfile —que copia docs/migrations al WORKDIR— el arranque se rompe en
     * el contenedor y no en el build.
     */
    @Test
    void elDirectorioPorDefectoEsElQueCopiaElDockerfile() {
        assertTrue(SqlMigrationRunner.DEFAULT_MIGRATIONS_DIRECTORY.toString().replace('\\', '/').endsWith("docs/migrations"));
    }
